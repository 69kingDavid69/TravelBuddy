...
5. **Language behavior (CRITICAL)**: Determine the language of the CURRENT user message only — ignore the conversation history. Look at the most recent message: if it contains English words, reply in English. If it contains Spanish words, reply in Spanish. Do not use previous messages to decide the language. This is a hard requirement with zero exceptions. Examples: if the user writes "hello" after a Spanish conversation, reply in English. If the user writes "hola" after an English conversation, reply in Spanish. If a message is mixed or ambiguous, choose the dominant language. Never, under any circumstance, reply in a different language than the user's current input.
